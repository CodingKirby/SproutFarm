...
public class Animal : MonoBehaviour
{
    ...
    private void SetSortingOrder()
    {
        ...
        // Assign sorting order based on the animal name
        if (gameObject.name.StartsWith("Cow"))
        {
            spriteRenderer.sortingOrder = 2000 + animalCount; // Higher order for Cow
        }
        else if (gameObject.name.StartsWith("Chicken"))
        {
            spriteRenderer.sortingOrder = 1000 + animalCount; // Lower order for Chicken
        }
        else
        {
            spriteRenderer.sortingOrder = 1500 + animalCount; // Default for other animals
        }

        // Ensure unique sorting orders
